...
export const Route = createFileRoute("/tasks")({
  head: () => ({
    meta: [
      { title: "Tasks — Smart Task Planner" },
      { name: "description", content: "Browse, filter and manage all your tasks." },
    ],
  }),
  component: TasksPage,
});
...
function TasksPage() {
  ...
  const filters: { id: typeof c.filter; label: string; count: number }[] = [
    { id: "all", label: t("filters.all"), count: c.stats.all },
    { id: "active", label: t("filters.active"), count: c.stats.active },
    { id: "completed", label: t("filters.completed"), count: c.stats.completed },
  ];
  return (
    <div className="mx-auto max-w-5xl space-y-5 p-4 sm:p-6">
      <header className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight">{t("pages.tasks.title")}</h1>
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border bg-card p-2 shadow-sm">
        ...
        <div className="flex items-center gap-1 rounded-lg bg-muted/40 p-1">
          {filters.map((f) => (
            ...
            >
              {f.label}
              <span className="rounded bg-muted/60 px-1 text-[10px]">{f.count}</span>
            </button>
          ))}
        </div>
        <select
          value={c.filterCategory}
          onChange={(e) => c.setFilterCategory(e.target.value as string | "all")}
        ...
        <select
          value={c.filterPriority}
          onChange={(e) => c.setFilterPriority(e.target.value as Priority | "all")}
          className="h-9 rounded-lg border bg-background px-2 text-xs"
        >
          <option value="all">{t("filters.all_priorities")}</option>
